...
export function ShareProvider({ slug, children }: { slug: string; children: ReactNode }) {
  ...
  useEffect(() => {
    if (share?.slug && slug !== share.slug) {
      router.replace(`/share/${share.slug}${path ? `/${path}` : ''}`);
    }
  }, [share?.slug, slug, router, path]);
  ...
  if (!isLoading && !isFetching && !share) {
    return (
      ...
      >
        <h2 style={{ margin: 0, fontWeight: 600 }}>Share not found</h2>
  ...
  return <ShareContext.Provider value={{ ...share, slug: share?.slug || slug }}>{children}</ShareContext.Provider>;
}
